...
object Par {
	...
	/**
	 * Feeback:
	 * I would have avoided using explicit recursion in sequence, even though it
	 * is very clear what your code does, simply because it can be a little cognitively
	 * difficult. For reference, here's my version:
	 * def sequence[A] (ps: List[Par[A]]): Par[List[A]] =
	...
	/**
	 * Very cool implementation of map3!
	 */
	...
	def choiceViaChooser[A] (cond: Par[Boolean]) (t: Par[A], f: Par[A]) : Par[A] = es =>
		run(es) (chooser(if(run(es) (cond).get) unit(0) else unit(1))(List(t,f)))
	/**
	 * Feedback:
	 * Your choiceViaChooser is quite verbose. Here's my version for comparison:
